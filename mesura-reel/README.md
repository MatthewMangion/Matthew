# Mesura.ai brand reel

A 26 second, 1920×1080, 60 fps motion piece for Mesura.ai. It's built in code, so every
...
- **Motion blur.** Each output frame averages 4–64 sub-frames across a 200° shutter
...
- **Sound.** `audio/score.py` synthesises a 100 BPM score in D (Bm9 → Gmaj9 → D/F# → Em9 →
  Bm → D → A/C# → F#m7 → Gmaj9 → Asus4 → Dmaj9) plus the sound design. Glyph reveals play a
  rising pentatonic run, odometer ticks follow the actual digit changes, the scan tone pans
  with the ruler, and the final full stop lands with the opening *tock*, a fifth higher. The
  measure and the mark both land on downbeats, and the suspended A resolves into D exactly
  as the mark forms.
